async function deployDNSSEC(web3, accounts, ens) {
  ...
  await ens.methods.setSubnodeOwner(
  ...
  await dnssec.methods.setAlgorithm(5, rsasha1._address).send({from:accounts[0]});
  await dnssec.methods.setAlgorithm(7, rsasha1._address).send({from:accounts[0]});
  await dnssec.methods.setAlgorithm(8, rsasha256._address).send({from:accounts[0]});
  await dnssec.methods.setDigest(1, sha1digest._address).send({from:accounts[0]});
  await dnssec.methods.setDigest(2, sha256digest._address).send({from:accounts[0]});
  await dnssec.methods.setNSEC3Digest(1, sha1nsec3digest._address).send({from:accounts[0]});
  return { dnssec }
}
